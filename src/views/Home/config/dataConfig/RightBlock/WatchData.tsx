...
import WatchDataTop from "./WatchDataTop";
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const delRow = () => {
      ...
      var transaction = store.db.transaction(["watchData"], "readwrite");
      transaction.oncomplete = () => {
        msg.success('删除成功')
        getTData()
      }
      var objectStore = transaction.objectStore("watchData");
    ...
    const getTData = () => {
      let transaction = store.db.transaction(['watchData'])
      var objectStore = transaction.objectStore('watchData')
    ...
    return () => {
      return (
        <div class={'w-full h-full flex flex-col'} id={'watchDataTableCon'}>
          {
            curTabValue.value == 'watchData' && isMount.value && <Teleport to="#dataConfigRightBlock">
